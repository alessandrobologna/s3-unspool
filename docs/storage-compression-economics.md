...
This document explains an important use case for `s3-unspool`: storing a
compressible corpus as one or more ZIP objects in S3, then extracting only
matching entries on demand with include and exclude globs.

It is written for someone deciding whether compressed archive storage is a good
fit for a real S3 workload. The numbers are scenario models, not a pricing
guarantee.
...
The case is valid when the corpus is mostly read-only, compresses well, and is
usually accessed by prefix, glob, project, date, or other grouped selection. It
is not a universal replacement for storing files as individual S3 objects. If
callers need low-latency direct `GetObject` access to each file, independent
per-file metadata, per-file lifecycle rules, event notifications, or frequent
single-file updates, individual S3 objects are still the simpler shape. If
callers need direct access to whole files and can store each file compressed
individually, compressed individual objects may be simpler than ZIP aggregation.

Use this page to reason about the storage model. Use
[Benchmark Snapshots](benchmark.md) to inspect measured Lambda extraction
behavior, and use the [Architecture](architecture.md) page to understand how
selection maps to ranged reads.
...
For 8 KiB and 32 KiB files, ZIP-in-Standard is clearly cheaper than the
individual-object alternatives. For 128 KiB files with very low access, Glacier
Instant Retrieval can be cheaper because the small-object minimum no longer
inflates the billable size. That advantage narrows or disappears as compression
improves or access rises.
...
Intelligent-Tiering has a different issue: files below 128 KiB stay in the
...
For 1 TiB, multiply the 100 GiB examples by about 10.24 when the object-size and
access assumptions stay the same.
...
The 1 TiB examples show the same pattern at a more meaningful scale. ZIP
aggregation wins strongly for tiny files. Glacier Instant Retrieval can be the
pure storage-cost winner for 128 KiB files at very low access, but it becomes
less attractive as reads increase because GET and retrieval fees are higher.
...
This is a different shape from the tiny-file case. At 1% monthly access, Glacier
Instant Retrieval is cheaper in this model because there is no small-object
penalty and retrieval volume is low. As access rises, retrieval fees matter
more, and compressed ZIPs in S3 Standard become more attractive.
...
For local extraction or extraction to a compute worker, request costs are usually
small compared with storage. For 100 GiB of 8 KiB files, reading 1% of the files
means about 131,000 selected files:
...
Lambda compute is usually not the cost blocker. At 512 MB, a conservative
...
storage. That cost does not apply when extracting to local disk, and it may also
apply to competing workflows if they materialize derived output.
...
- Files are compressible text, structured data, source artifacts, logs, reports,
  or bundles.
- Files are small enough that object-count effects matter, or large enough that
  compression meaningfully reduces stored and read bytes.
...
| 8:1 | already cheaper in this model |
...
storage classes." The honest claim is narrower:
